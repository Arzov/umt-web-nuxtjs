  ...
  nearMatches (ctx, data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.$AWS.API.graphql(
        ...
        .then(async (result) => {
          const nearMatches = result.data.nearMatches.items.map((x) => {
            return {
              ...x,
          ...
          const nearMatchesNextToken = result.data.nearMatches.nextToken

          if (nearMatches) {
            for (const e in nearMatches) {
              for (let i = 1; i <= 2; i++) {
                await this.$AWS.API.graphql(
                  graphqlOperation(umt.queries.getTeam, { id: nearMatches[e][`teamId${i}`] })
                )
                  .then((result) => {
